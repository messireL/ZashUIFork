...
const sankeyData = computed(() => {
  ...
  for (const c of conns) {
    ...
    addLink(rootName.value, source, 1)
    addLink(source, chain0, 1)
    if (chain1) addLink(chain0, chain1, 1)
  }
  ...
  series: [
    {
      ...
      label: { color: colorSet.baseContent, fontFamily, fontSize: 9 },
    },
  ],
}))
...
onMounted(() => {
  ...
  watch([activeUuid, options, isFullScreen], () => {
    ...
    if (isFullScreen.value) {
      nextTick(() => {
        if (!fullScreenMyChart.value) fullScreenMyChart.value = echarts.init(fullScreenChart.value)
